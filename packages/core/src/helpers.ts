...
export const replaceTemplatedText = (
	...
	return text.replace(/{{(.*?)}}/g, (match: string) => {
		const [attribute, ...formatters] = match
			...
			.split("|")
			.map((s) => s.trim());
		const parentScopedAttribute = data?.["@parent"]
			? attribute.replace(`${data["@parent"]}/`, "")
			: attribute;
		const stateItem = state?.find(
			(state) => state.id === parentScopedAttribute || state.id === attribute,
		);
		const value =
			replacements[parentScopedAttribute] ??
			replacements[attribute] ??
			stateItem?.value;

		return (
			formatValue(value, { formatters, type: stateItem?.type, locale }) ?? "..."
		);
	});
};
 ...
 */
export const isAttributeDynamic = (
