...
const ENTITY_LIST_SIDEBAR_DATA_INFO: SidebarDynamicDataInfo<RenderableEntityListSidebar> =
	{
		...
		generateData: (config, response) => {
			...
			const attributes = Array.isArray(response.reporting)
				? response.reporting
				: [response.reporting];
			const entities = attributes.find(
				(attributes: any) => attributes.id === `entity_list_${config.entity}`,
			);
