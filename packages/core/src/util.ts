...
export const isStrNotNullOrBlank = (str: any): boolean =>
	!/^\s*$/.test(str || "");
export const isStrNullOrBlank = (str: any): boolean =>
	!isStrNotNullOrBlank(str);
	...
	if (Array.isArray(obj)) {
		for (let i = 0; i < obj.length; i++) {
			flattenObject(
				obj[i],
				delimiter,
				parentKey ? `${parentKey}${delimiter}${i}` : `${i}`,
				result,
			);
		}
	} else {
		...
		for (const [key, value] of Object.entries(obj)) {
			flattenObject(
				value,
				delimiter,
				parentKey ? `${parentKey}${delimiter}${key}` : key,
				result,
			);
		}
	}
...
export const instanceControl = (
	control: RenderableEntityControl,
	id: string,
): EntityControlInstance => {
	const controls =
		control.instances?.find((instance) => instance.id === id)?.controls ??
		structuredClone(control.template);
	iterateControls(controls, (instanceControl: any) => {
		instanceControl.id = uuid();
		if (typeof instanceControl.templateText === "string") {
			instanceControl.templateText = instanceControl.templateText.replace(
				/@id/g,
				id,
			);
		}
		...
		if (Array.isArray(instanceControl.dynamicAttributes)) {
			instanceControl.dynamicAttributes = instanceControl.dynamicAttributes.map(
				(attr: string) => attr.replace(/@id/g, id),
			);
		}
		if (instanceControl.type === "entity") {
			...
			instanceControl.instances = keys.map((key) =>
				instanceControl(instanceControl, key),
			);
		}
	});
...
export const applyInstancesToEntityControl = (
	control: RenderableEntityControl,
	instances: string[],
) => {
...
export const attributeToPath = <S extends string | undefined>(
	...
	const basePath =
		parent && attribute.startsWith(`${parent}/`)
			? attribute.replace(`${parent}/`, "")
			: attribute;
...
export const postProcessControl = (
	...
	if (control.templateText) {
		control.text = replaceTemplatedText(
			control.templateText,
			replacements,
			data,
			state,
			locale,
	...
	if (control.templateLabel) {
		control.label = replaceTemplatedText(
			control.templateLabel,
			replacements,
			data,
			state,
			locale,
	...
	if (
		control.type === "switch_container" &&
		control.kind === "dynamic" &&
		control.attribute
	) {
	...
	if (control.type === "certainty_container") {
		...
		if (update !== undefined) {
			control.branch =
				replacements[control.attribute] === null ? "uncertain" : "certain";
		}
	}
};
