...
import {get, isEmpty, isEqual, debounce} from "lodash-es";
...
import { applyInstancesToEntityControl, buildUrl, createEntityPathedData, iterateControls, range } from "./util";
...
export class SessionInstance implements Session {
    ...
    if (state && this.screen) {
      ...
      if (
        ...
        if (replacementQueries.unknownValues.length || Object.keys(replacementQueries.knownValues).length > 0) {
          ...
          if (this.debug) {
            console.log("[@decisively-io/interview-sdk] DEBUG: Calculated knowns:", replacementQueries?.knownValues);
            console.log(
          ...
          iterateControls(newScreen.controls, (control: any) => {
            ...
            if (!control.loading) {
              postProcessControl(control, this.internals.replacements, this.data, this.state, this.session.locale);
            }
          });
  ...
  private async updateDynamicValues() {
    ...
    if (Object.keys(this.internals.unknownsRequiringSimulate).length > 0 && this.session.screen) {
      ...
      const result = await simulate(Object.values(this.internals.unknownsRequiringSimulate), this.api, this);
    ...
    if (this.internals.sidebarSimulate) {
      const result = await postSimulate(this.api, this, this.internals.sidebarSimulate.simulate);
      for (const sidebarId of this.internals.sidebarSimulate.ids) {
        ...
        if (screenSidebar) {
          const dataInfo = (SIDEBAR_DYNAMIC_DATA_INFO as any)[screenSidebar.type];
          if (dataInfo) {
            try {
