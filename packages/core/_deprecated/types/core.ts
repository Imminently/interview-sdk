...
export const isFileAttributeValue = (v: unknown): v is FileAttributeValue => {
	...
	if (
		Array.isArray(typed.fileRefs) === false ||
		typed.fileRefs.some((it) => it.indexOf("data:id=") !== 0)
	) {
		return false;
	}
...
const decodeFromBase64 = (base64Str: string) =>
	new TextDecoder().decode(__base64ToBytes(base64Str));

export const getNameFromFileAttributeRef = (
	ref: FileAttributeValue["fileRefs"][0],
) => {
...
export const getIdFromFileAttributeRef = (
	ref: FileAttributeValue["fileRefs"][0],
