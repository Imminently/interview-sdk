...
export const defaultRenderControl: any = (control: ControlDef, classNames: any, renderControl: any, overrides: any) => {
  ...
  if (control.type === "text") {
    return (
      <div
        key={control.id}
        className={clsx("dcsvly-ctrl-text", classNames.Text.root)}
      >
        <label className={clsx("dcsvly-ctrl-text-label", classNames.Text.label)}>
          {control.label}
          <input
            className={clsx("dcsvly-ctrl-text-input", classNames.Text.input)}
            type="text"
            name={control.id}
...
export const ControlRenderer = (props: any) => {
  ...
  if (controlOverrides && controlOverrides[control.type])
    return controlOverrides[control.type](control, classNames, controlOverrides);
  switch (control.type) {
    case "text":
      return (
        <TextControl
