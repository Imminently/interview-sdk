...
export const useCombobox = (control: OptionsControl, debounceMs = 300) => {
  ...
  const fetchData = useCallback(
    async (val: string) => {
      ...
      const {
        asyncOptions: { minInput, query, ...options },
      } = control;
...
const ComboControl = (props: any) => {
  ...
  return (
    ...
    >
      ...
      <ComboboxContent>
        <ComboboxInput
          placeholder={t("form.combobox.search_placeholder")}
          name={field.name}
          value={search}
          onValueChange={setSearch}
        />
        <ComboboxEmpty>{loading ? t("form.combobox.loading") : t("form.combobox.no_options")}</ComboboxEmpty>
        <ComboboxList>
          <ComboboxGroup>
            {options.map((item) => (
              <ComboboxItem
